...
\begin{document}
  ...
  \begin{abstract}
    ...
    Beyond the mixing question, the same stratum settles the generation-scale question negatively:
    every depth read from the static admissibility spectrum --- crossing, saturation, capacity cell,
    quantised count, and quantised resolution density --- yields only order-one ratios or re-introduces the
    target scale through an unforced normalisation, so the static spectral stratum fixes the generation
    structure and the order-one level split, but not the charged-fermion hierarchy.
  ...
  \section{Polar class of the projected Yukawa operator}
    ...
    \begin{corollary}[Real-cascade collapse]
      ...
      defect being the real $V\!-\!A$ diagonal split~\cite{Beau2026prs}, and the odd Lorentzian deformation does not
      switch it on at first order, the first-order generation-mixing coefficient of $\Epi^{2}$ vanishing by antiunitary
      parity in every covariant family, even orders not being excluded~\cite{Beau2026ebj}; hence $A$ is diagonal,
